// backend/database/migrate-test.js
...
async function migrateTest() {
  ...
  const dbName = process.env.DB_NAME || 'pawshop_test';
  
  await conn.query(`DROP DATABASE IF EXISTS ${dbName}`);
  await conn.query(`CREATE DATABASE ${dbName} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
  await conn.query(`USE ${dbName}`);
  
  // Đọc và chạy migrate.js
  const migrateSQL = fs.readFileSync(path.join(__dirname, 'migrate.js'), 'utf8');
  // Không chạy trực tiếp, thay vào đó tạo bảng thủ công
