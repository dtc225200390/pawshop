// backend/tests/setup/setup-db.js
...
const TEST_DB_NAME = process.env.DB_NAME || 'pawshop_test';
...
async function setupTestDB() {
  ...
  // Đọc file migrate.js và chạy
  const migratePath = path.join(__dirname, '../../database/migrate.js');
  if (fs.existsSync(migratePath)) {
    console.log('📦 Running migrate.js...');
    // Chạy migration bằng cách require và thực thi
    const migrate = require(migratePath);
    if (typeof migrate === 'function') {
      await migrate();
    }
  } else {
    console.log('⚠️  migrate.js not found, creating tables manually...');
